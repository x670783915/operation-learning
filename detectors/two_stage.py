...
class TwoStageDetector(BaseDetector, RPNTestMixin, BBoxTestMixin, MaskTestMixin):
    def __init__(
        ...
        if neck is not None:
            self.neck = builder.build_neck(backbone)
        ...
        if bbox_head is not None:
            self.bbox_roi_extractor = builder.build_roi_extractor(bbox_roi_extrctor)
        ...
        if mask_head is not None:
            if mask_roi_extractor is not None:
                ...
                self.mask_roi_extractor = bbox_roi_extrctor
    ...
    def init_weights(self, pretrained=None):
        ...
        if self.with_shared_head:
            self.share_head.init_weights(pretrained=pretrained)
        if self.with_rpn:
