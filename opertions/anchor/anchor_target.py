...
def anchor_target_single(
    ...
    if sampling:
        assign_res, sampling_res = assign_and_sample(
            anchors, gt_bboxes, gt_bboxes, None, cfg
        )
    else:
        bbox_assigner = build_assigner(cfg.assigner)
        assign_res = bbox_assigner.assign(anchors, gt_bboxes, gt_bboxes_ignore, gt_labels)
        bbox_sampler = PseudoSampler()
        sampling_res = bbox_sampler.sample(assign_res, anchors, gt_bboxes)
    ...
    pos_inds = sampling_res.pos_inds
    neg_inds = sampling_res.neg_inds
    if len(pos_inds) > 0:
        pos_bbox_targets = bbox2delta(
            sampling_res.pos_inds,
        ...
        if gt_labels is None:
            ...
            labels[pos_inds] = gt_labels[sampling_res.pos_assigned_gt_inds]
        if cfg.pos_weight <= 0:
            label_weights[pos_inds] = 1.0
        else:
            label_weights[neg_inds] = cfg.pos_weight
    if len(neg_inds) > 0:
